...
Later, specifically in Chapters @sec-multivariate-regression and @treatment-effect-models, we will see how the linear model representation permits us to use the same mathematical frameworks in other contexts and to achieve more complicated goals than predict one variable from another.
...
In Chapter @sec-multivariate-regression, after describing a more complex case study, we learn more about applying regression in R.
...
In Chapters @sec-multivariate-regression and @sec-treatment-effect-models we introduce data analysis challenges in which more than one variables some not included in the model. In these cases an important diagnostic test to add checks if the residuals are related to variables not included in the model.
