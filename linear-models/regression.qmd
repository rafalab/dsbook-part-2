...
Later, specifically in Sections @sec-multivariate-regression and @sec-treatment-effect-models, we will see how the linear model representation permits us to use the same mathematical frameworks in other contexts and to achieve more complicated goals than predicting one variable from another.
...
We prefer plots rather than summaries based on, for example, correlation because, as noted in Section @sec-ascombe, correlation is not always the best summary of association. The function `plot` applied to an `lm` object automatically plots these.
...
In @sec-multivariate-regression and @sec-treatment-effect-models, we introduce data analysis challenges in which we may decide to not include certain variables in the model. In these cases, an important diagnostic test is to check if the residuals are related to variables not included in the model.
