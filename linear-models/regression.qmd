...
galton[, .(mean(father), sd(father), mean(son), sd(son))]
```
...
The correlation coefficient is always between -1 and 1. We can show this mathematically, and we include it as an exercise for you to prove.
...
Because `r` is a random variable, we can run a Monte Carlo simulation to see its distribution:
...
Galton wanted to understand how well we can predict a child's height from the parents' heights. To begin, we frame this as a prediction problem.
...
It turns out that, if we were able to collect data from a very large number of fathers who are 72 inches, the distribution of their sons' heights would be normally distributed. This implies that the average of the distribution computed on this subset would be our best prediction.
...
We write $Y \mid X = x$ (read "$Y$ given, or conditional on, $X = x$") to mean a random value drawn from the distribution of $Y$ among pairs where the event $X = x$ holds. In other words, $Y \mid X = 72$ represents the random height of a son chosen from among those whose fathers are 72 inches tall.
...
where $x$ is the fixed value defining the subset. Similarly, we denote the standard deviation within that subset, or stratum, as:
...
A practical way to improve estimates of the conditional expectations is to define strata of observations with similar values of $x$. In our example, we can round father heights to the nearest inch to pool observations with father heights close to 72 inches. If we do this, we end up with the following prediction for the son of a father who is 72 inches tall:

```{r}
mean(galton[round(father) == 72]$son)
```

Note that a 72-inch father is taller than average, specifically (72.0 - `r round( mean(galton$father),1)`)/`r round(sd(galton$father),1)` = `r round((72 -mean(galton$father))/sd(galton$father), 1)` standard deviations taller than the average father. Our prediction `r galton[round(father) == 72, mean(son)]` is also taller than average, but only `r round((galton[round(father) == 72, mean(son)] - mean(galton$son)) /sd(galton$son),2)` standard deviations larger than the average son. The predicted height for sons of 72-inch fathers has *regressed* towards the average height. We notice that the reduction in how many SDs taller is about 0.5, which happens to be close to the sample correlation. As we will see in a later section, this is not a coincidence.

If we want to make a prediction of any height, not just 72 inches, we could apply the same approach to each stratum. Stratification followed by boxplots lets us see the distribution of each group:
...
If we are predicting a random variable $Y$ knowing the value of another $X=x$ using a regression line, then we predict that for every standard deviation, $\sigma_X$, that $x$ increases above the average $\mu_X$, our prediction $\hat{y}$ increases $\rho$ standard deviations $\sigma_Y$ above the average $\mu_Y$ with $\rho$ the correlation between $X$ and $Y$. The formula for the regression is therefore:
...
If there is a perfect correlation, the regression line predicts an increase that is the same number of SDs. If there is 0 correlation, then we don't use $x$ at all for the prediction and simply predict the average $\mu_Y$. For values between 0 and 1, the prediction is somewhere in between. If the correlation is negative, we predict a reduction instead of an increase.

Note that if the correlation is positive and lower than 1, our prediction is closer, in standard units, to the average height than the value used to predict, $x$, is to the average of the $x$s. This is why we call it *regression*: the sons regress to the average height. In fact, the title of Galton's paper was: *Regression toward mediocrity in hereditary stature*. 

We can add regression lines to plots by converting the formula above to the form $\hat{y} = mx + b$, which gives us a slope $m = \rho \frac{\sigma_y}{\sigma_x}$ and 
...
```{r}
#| eval: false
galton |> ggplot(aes(father, son)) + geom_point() +
...
Note that the regression formula implies that if we first standardize the variables, that is, subtract the average and divide by the standard deviation, then the regression line has an intercept of 0 and a slope equal to the correlation $\rho$. You can make the same plot, but using standard units like this:
...
1. Round fathers' heights to the nearest inch, stratify, and take the average son's height within each group.
2. Compute the regression line and use it to predict the son's height.

For each method, our goal is to estimate the expected value of a son's height given that his father is 72 inches tall, as well as the standard error of these estimates. We want the estimated conditional expected value to be very close to the true conditional population average, and we want the standard error to be small. To do this, we use a Monte Carlo simulation. Specifically, we repeatedly draw random samples of size $N = 50$ families from the population, compute both types of predictions for each sample, and then examine the distribution of these repeated estimates. The mean of this distribution provides an estimate of the expected value of each method, and its standard deviation provides an estimate of the standard error, which reflects how variable the prediction is across random samples.
...
linear_regression_prediction <- replicate(B, {
  dat <- galton[sample(.N, N, replace = TRUE)]
  dat[, mean(son) + cor(father, son)*(72 - mean(father))/sd(father)*sd(son)]
})
```
...
This means that the linear-regression-based prediction is far more stable across repeated samples. The reason is intuitive: the within-stratum sample mean is based on a relatively small subset of data, the fathers whose heights are about 72 inches, so its estimate is more variable. In some samples, we may even have very few or no observations in that group, which is why we use `na.rm=TRUE`. The regression line, on the other hand, is estimated using information from the entire dataset, which makes its predictions much more precise.

So why not always use the regression line for prediction? Because it is not always appropriate. For example, Anscombe provided cases for which the data do not have a linear relationship. So are we justified in using the regression line to predict? Galton answered this in the positive for height data. The next section provides a justification.
...
If we think the height data is well approximated by the bivariate normal distribution, then we should see the normal approximation hold for each stratum. Here we stratify the son heights by the standardized father heights and see that the assumption appears to hold:
...
This implies that, if our data is approximately bivariate normal, the regression line gives the conditional expectation of $Y$ given $X$. Therefore, we can obtain a much more stable estimate of the conditional expectation by fitting the regression line and using it for prediction.
...
To see why this is intuitive, notice that without conditioning,  we are looking at the variability of all the sons:  $\mathrm{SD}(Y) = \sigma_Y$. But once we condition, we are only looking at the variability of the sons with a tall, 72-inch father. Because the variables are positively correlated, this group will all tend to be somewhat tall, so the standard deviation is reduced.

Specifically, it is reduced to $\sqrt{1-\rho^2} = \sqrt{1 - `r round(galton[, cor(father, son)]**2, 2)`} = `r round(sqrt(1-galton[, cor(father, son)]**2), 2)`$ of what it was originally. We could say that fathers' heights "explain" `r 100*(1-round(sqrt(1-galton[, cor(father, son)]**2), 2))`% of the standard deviation observed in son heights.
...
But it is important to remember that the _variance explained_ statement has this clear interpretation only when the data is approximated by a bivariate normal distribution.

### There are two regression lines

We computed a regression line to predict the son's height from the father's height. We used these calculations:
...
Using these calculations we get $\mathrm{E}[X \mid Y=y] = `r round(b_2, 1)` + `r round(m_2, 2)`y$. Again, we see regression to the average, indicated by a slope between 0 and 1: the predicted father height is closer to the average father height than the observed son’s height is to the average son height.

Here is a plot showing the two regression lines, with blue for predicting son heights with father heights, and red for predicting father heights with son heights:
...
So is it "jitters" or a "jinx"? To answer this question, let's turn our attention to all the players who played the 2013 and 2014 seasons and batted more than 130 times (minimum to win Rookie of the Year).
...
3\. We performed a Monte Carlo simulation and found that with $N=25$, the CLT was not a great approximation for the distribution of the sample correlation. Generate correlated data using the `mvrnorm` function from the **MASS** package with correlation 0.5:
...
N <- 25
rho <- 0.5
dat <- mvrnorm(N, c(0, 0), matrix(c(1, rho, rho, 1), 2, 2))
cor(dat[,1], dat[,2])
```
...
6\. Load the `GaltonFamilies` data from the **HistData** package. The children in each family are listed by gender and then by height. Create a dataset called `galton` by restricting to families with at least one daughter and at least one son. Then, for each family, select a single male and a single female child at random.

7\. Make separate scatterplots for heights between mothers and daughters, mothers and sons, fathers and daughters, and fathers and sons.
