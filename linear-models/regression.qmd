...
set.seed(1983)
galton_heights <- as.data.table(GaltonFamilies)[gender == "male", .SD[sample(.N, 1)], by = family]
galton_heights <- galton_heights[, .(father, son = childHeight)]
```
...
galton_heights[, .(father_avg = mean(father), father_sd = sd(father), son_avg = mean(son), son_sd = sd(son))] 
```
...
To see why this equation captures how two variables move together, note that the $i$-th entry of $x$ represents how many standard deviations $\left(\frac{x_i - \mu_x}{\sigma_x}\right)$ the value $x_i$ is from the mean. The corresponding $y_i$ shows how many standard deviations $\left(\frac{y_i - \mu_y}{\sigma_y}\right)$ it is from the mean of $y$. If $x$ and $y$ are unrelated, the product $\left(\frac{x_i - \mu_x}{\sigma_x}\right)\left(\frac{y_i - \mu_y}{\sigma_y}\right)$ will be positive (positive times positive or negative times negative) about as often as it is negative (positive times negative or negative times positive). These positive and negative products cancel out when averaged, giving a correlation close to zero. If $x$ and $y$ tend to increase and decrease together, most of these products will be positive, producing a positive correlation. If one tends to increase when the other decreases, most products will be negative, producing a negative correlation.

The correlation coefficient is always between -1 and 1. We can show this mathematically: consider that we can't have higher correlation than when we compare a two observations that follow a perfect linear relationship, say $y_i = a + bx_i$, and, in this case, the correlation is:

$$
\begin{aligned}
\rho &= \frac{1}{n} \sum_{i=1}^n \left( \frac{x_i-\mu_x}{\sigma_x} \right) \left( \frac{y_i-\mu_y}{\sigma_y} \right) \\
&= \frac{1}{n} \sum_{i=1}^n \left( \frac{x_i-\mu_x}{\sigma_x} \right) \left( \frac{a+bx_i-a - b\mu_x}{b\sigma_x}\right) \\
&= \frac{1}{n} \sum_{i=1}^n \left( \frac{x_i-\mu_x}{\sigma_x} \right) \left( \frac{b(x_i- \mu_x)}{b\sigma_x}\right) \\
&=\frac{1}{n} \sum_{i=1}^n \left( \frac{x_i-\mu_x}{\sigma_x} \right)^2\\ 
&= \frac{1}{\sigma_x^2} \frac{1}{n} \sum_{i=1}^n \left( x_i-\mu_x \right)^2 \\
&= \frac{1}{\sigma_x^2} \sigma^2_x = 1
\end{aligned}
$$

A similar derivation, but with $y_i = a - bx_i$, proves the correlation has to be bigger or equal to -1.
...
```{r}
r <- galton_heights[sample(.N, 25, replace = TRUE), cor(father, son)]
```
...
B <- 1000
N <- 25
r <- replicate(B, galton_heights[sample(.N, 25, replace = TRUE), cor(father, son)])
hist(r, breaks = 20)
```
...
```{r small-sample-correlation-not-normal, out.width="40%", echo=FALSE}
...
To describe this mathematically, suppose we have a population of pairs $(x_1, y_1), \dots, (x_n, y_n)$, such as all father and son height pairs in England. In the previous chapter, we learned that if we pick a random pair $(X, Y)$ from this population, the expected value and best predictor of $Y$ is $\mathrm{E}[Y] = \mu_y$, the population average, given by $1/n \sum_{i=1}^n y_i$.

Now, instead of the entire population, we focus on a specific subgroup defined by a fixed value of $X$. In our example, this might be all pairs for which the father's height is 72 inches. This subgroup is itself a population, so the same ideas apply. The $y_i$ values in this subgroup follow a distribution called the *conditional distribution*, and this distribution has an expected value called the *conditional expectation*. In our example, the conditional expectation is the average height of all sons whose fathers are 72 inches tall.

When we write $Y \mid X = x$, we mean a random value of $Y$ selected only from the pairs where the event $X = x$ holds. In other words, $Y \mid X = 72$ represents the random height of a son chosen from among those whose fathers are 72 inches tall.
...
where $x$ is the fixed value defining the subset, for example 72 inches. Similarly, we denote the standard deviation within that subset, or strata, as:
...
We previously learned that the sample average is the preferred approach to estimating the population average. But it is not going to be the best way to estimate conditional expectations when X and Y are correlated. We want to estimate  $E[Y|X=72]$ using the sample collected by Galton. When using continuous data we often have the challenge that there are not that many data points where $X = 72$: 
...
```{r}
galton_heights[round(father) == 72, mean(son)]
```

Note that a 72 inch father is taller than average, specifically (72.0 - `r round( mean(galton_heights$father),1)`)/`r round(sd(galton_heights$father),1)` = `r round((72 -mean(galton_heights$father))/sd(galton_heights$father), 1)` standard deviations taller than the average father. Our prediction `r galton_heights[round(father) == 72, mean(son)]` is also taller than average, but only `r round((galton_heights[round(father) == 72, mean(son)] - mean(galton_heights$son)) /sd(galton_heights$son),2)` standard deviations larger than the average son. The sons of 72 inch fathers have *regressed* some to the average height. We notice that the reduction in how many SDs taller is about 0.5, which happens to be close to the sample correlation. As we will see in a later section, this is not a coincidence.
...
galton_heights[, .(father_strata = factor(round(father)), son)] |>
...
galton_heights[, .(father= factor(round(father)), son)][, .(avg = mean(son)), by = father] |>
...
The fact that these conditional averages follow a line is not a coincidence. In the next section, we explain that this line, known as the *regression line*, improves the precision of our estimates. However, it is not always appropriate to estimate conditional expectations with the regression line, so we also describe Galton's theoretical justification for using it.
...
params <- galton_heights[, .(mu_x = mean(father), mu_y = mean(son),
...
conditional_avg <- replicate(B, {
  dat <- galton_heights[sample(.N, N, replace = TRUE)]
...
the standard error for the regression prediction is much smaller:
...
This means that the regression-based prediction is far more stable across repeated samples. The reason is intuitive: the conditional average is based on a relatively small subset of data, the fathers whose heights are about 72 inches, so its estimate is more variable. In some samples, we may even have very few or no observations in that group, which is why we use `na.rm=TRUE`. The regression line, on the other hand, is estimated using information from the entire dataset, which makes its predictions much more precise.
...
```{r qqnorm-of-strata}
galton_heights[, .(son, z = round((father - mean(father))/sd(father)))][z %in% -2:2] |>
  ggplot() +  
...
To see why this is intuitive, notice that without conditioning, $\mathrm{SD}(Y) = \sigma_Y$, we are looking at the variability of all the sons. But once we condition, we are only looking at the variability of the sons with a tall, 72 inch father. This group will all tend to be somewhat tall so the standard deviation is reduced.
...
So we get $\mathrm{E}[X \mid Y=y] =$ `r round(b_2, 1)` + `r round(m_2, 2)`y. Again, we see regression to the average: the prediction for the father is closer to the father average than the son heights $y$ is to the son average.
...
B <- 10000
N <- 25
lse <- replicate(B, coef(lm(son ~ father, data = galton_heights[sample(.N, N, replace = TRUE)])))
...
```{r}
summary(lm(son ~ father, data = galton_heights[sample(.N, N, replace = TRUE)]))$coef
```
...
Remember that, as we described in @sec-t-dist, for large enough $N$, the CLT works and the t-distribution becomes almost the same as the normal distribution. Also, notice that we can construct confidence intervals, but we will soon learn about **broom**, an add-on package that makes this easy.
...
In Major League Baseball, the rookie of the year (ROY) award is given to the first-year player who is judged to have performed the best. The *sophomore slump* phrase is used to describe the observation that ROY award winners don't do as well during their second year. For example, this Fox Sports article[^linear-models-1] asks "Will MLB's tremendous rookie class of 2015 suffer a sophomore slump?"

[^linear-models-1]: https://en.wikipedia.org/w/index.php?title=Sophomore_slump&oldid=1313627773
...
The correlation is `r round(cor(two_years$"2013",two_years$"2014"),2)` and the data look very much like a bivariate normal distribution, which means we predict a 2014 batting average $Y$ for any given player that had a 2013 batting average $X$ with:
...
4\. Fit a regression model to each of the datasets in the Anscombe quartet and examine the diagnostic plots.
