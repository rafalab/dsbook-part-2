...
In the example above, the slope is `r  lm(r ~ hr, data = dat)$coef[[2]]`. This tells us that teams that hit 1 more HR per game than the average team, score `r lm(r ~ hr, data = dat)$coef[[2]]` more runs per game than the average team. Given that the most common final score is a difference of one run, this can certainly lead to a large increase in wins. Not surprisingly, HR hitters are very expensive. Because we are working on a budget, we will need to find some other way to increase wins. In the next chapter, we introduce _linear models_, which provide an framework for performing this analysis. In \@ref{@moneyball}, we apply what have learned to build a baseball team.
...
They are reduced slightly from `r lm(r ~ hr, data = dat)$coef[2]`, which is consistent with the fact that BB do in fact cause some runs.
