...
Can we use regression with these data? First, notice that the HR and Run data, shown above, appear to be bivariate normal. 
Specifically, the qq-plots confirm that the normal approximation for each HR strata is useful here:
...
In the example above, the slope is `r  lm(r ~ hr, data = dat)$coef[[2]]`. So this tells us that teams that hit 1 more HR per game than the average team, score `r lm(r ~ hr, data = dat)$coef[[2]]` more runs per game than the average team. Given that the most common final score is a difference of a run, this can certainly lead to a large increase in wins. Not surprisingly, HR hitters are very expensive. Because we are working on a budget, we will need to find some other way to increase wins. In the next chapter, we introduce _linear models_, which provide an framework for performing this analysis. In chapter \@ref{@moneyball} we apply what have learned to build a baseball team.
...
and returns it in a **tidyverse** friendly data frame.  These functions are `tidy`, `glance`, and `augment`. The `tidy` function returns estimates and related information as a data frame:
...
They are reduced a bit from `r lm(r ~ hr, data = dat)$coef[2]`, which is consistent with the fact that BB do in fact cause some runs.
...
When we fit the model with only one variable, the estimated slopes were `r lm(r ~ bb, data = dat)$coef[2]` and `r lm(r ~ hr, data = dat)$coef[2]` for BB and HR, respectively. Note that when fitting the multivariable model both go down, with the BB effect decreasing much more.
