...
But is this player not helping produce runs? While the highest AVG receives the *batting title* award, no award is given to the player with the most BB. However, bad habits are hard to break, and baseball did not immediately adopt OBP as an important statistic. We can use data to try to make a case that BB does indeed help produce runs and should be valued.
...
1\. In a previous section, we computed the correlation between heights of mothers and daughters, mothers and sons, fathers and daughters, and fathers and sons. We noticed that the highest correlation is between fathers and sons, and the lowest is between mothers and sons. We can compute these correlations using:
...
Are these differences statistically significant? To answer this, we will compute the slopes of the regression lines along with their standard errors. Start by using `lm` to compute the slopes LSE and the standard errors.

2\. Repeat exercise 1, but compute a confidence interval as well.

3\. Plot the confidence intervals and notice that they overlap, which implies that the data are consistent with the inheritance of height being independent of sex.

4\. In the code above, we used `sample_n(1)` to randomly select a child of each sex from each family. Because this step introduces randomness, the correlations we compute will vary if we rerun this code chunk. Repeat the computation of correlations 100 times, taking a different sample each time. Examine how much the resulting correlations vary across these repetitions. Hint: Use an approach similar to what we have done for simulations.
...
6\. You can use the `inner_join` function to combine the 2002 data and averages in the same table:
...
7\. Note that the correlation is higher for BB. To quickly get an idea of the uncertainty associated with this correlation estimate, we will fit a linear model and compute confidence intervals for the slope coefficient. However, first make scatterplots to confirm that fitting a linear model is appropriate.
...
12\.  Write a function that takes R, HR, and BB as arguments and fits two linear models: `R ~ BB` and `R~BB+HR`. Then use the `summary` function to obtain the `BB` for both models for each year since 1962. Then plot these against each other as a function of time.
...
21\. Perform careful model diagnostics to assess the assumptions of our model.
