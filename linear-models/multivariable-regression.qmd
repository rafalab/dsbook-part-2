...
But is this player not helping produce runs? While the highest AVG receives the *batting title* award, no award is given to the player with the most BB. However, bad habits are hard to break and baseball did not immediately adopt OBP as an important statistic. We can use data to try and make a case that BB do indeed help produce runs and should be valued.
...
with $Y_i$ runs per game for team $i$, $x_{i,1}$ BB per game, $x_{i,2}$ the HR per game, and $\varepsilon_i$ assumed to be independent and identically distributed. This fits into our linear model framework, whcih means we can use `lm` to fit the model.
...
This case study illustrates the power and versatility of multivariable regression. By extending the simple linear model framework, we were able to adjust for confounding variables and estimate the contribution of each factor while holding others constant. The same mathematical ideas introduced earlier, expectation, standard deviation, conditional distributions, underpin this analysis, and the same `lm` function in R provides the computational machinery to fit these models. In short, the Moneyball example demonstrates how the linear modeling tools we have developed throughout this part of the book can be applied to real-world data to make informed, evidence-based decisions.
...
6\. Repeat the exercise 5, but compute a confidence interval as well.
...
21\. Perform a careful model diagnostics to assess if the model described in @sec-run-production-model.
