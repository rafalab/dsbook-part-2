...
if the outcome data, in this case weight, follows an approximately normal distribution, then the $t$, defined above, follows a t-distribution with $N_0+N_1-2$ degrees of freedom. So the calculation of the p-value is the same except that we use `pt` instead of `pnorm`. Specifically, we use `1-pt(t_stat, with(stats, n[2]+n[1]-2))`. 
