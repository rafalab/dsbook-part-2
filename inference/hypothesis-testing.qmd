...
:::{.callout-note}
We use $\pi$ to represent the probability of drawing a blue bead, instead of $p$ as in previous sections, to avoid confusion with between the parameter $p$ and the p in p-value.
:::
...
Keep in mind that there is a close connection between p-values and confidence intervals. In our example, if a 95% confidence interval does not include 0.5, we know that the p-value must be smaller than 0.05. In general, we can show mathematically that if a $(1-\alpha)\times 100$% confidence interval does not contain the null hypothesis value, the null hypothesis is rejected with a p-value as small or smaller than $\alpha$. So _statistical significance_ can be determined from confidence intervals.
