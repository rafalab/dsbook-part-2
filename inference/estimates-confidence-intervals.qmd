...
The standard deviation of an estimate is often called its *standard error* (SE), and it quantifies how much the estimate is expected to vary from one sample to another. The smaller the standard error, the more precise our estimate is. 
...
14\. Note that the error $\bar{X}-p$ is a random variable. In practice, the error is not observed because we do not know $p$. Here, we observe it since we constructed the simulation. Using the Monte Carlo simulation, what is the average size of the error if we define the size by taking the absolute value $\mid \bar{X} - p \mid$?
