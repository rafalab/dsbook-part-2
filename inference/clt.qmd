...
:::{.callout-note}
We use $\mu$ the denote the spread here and in the next chapters because this is the typical notation used in statistical textbooks for the parameter of interest. The reason we use  $\mu$ is because a populuation mean is often the parameter of interest and $\mu$ is the Greek letter for _m_.
:::
