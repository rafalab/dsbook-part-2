...
data.frame(Spread = as.vector(y) , Pollster = as.factor(rep(1:I, each = J))) |>
  ggplot(aes(Pollster, Spread)) +
  geom_point() +
  scale_y_continuous(limits = range(tmp$spread)) +
  coord_flip()
```
...
- **Elliott Morris (2020).** *How to Build an Election Forecast Model.*  
  ...
  [https://projects.economist.com/us-2020-forecast/methodology](https://projects.economist.com/us-2020-forecast/methodology)
