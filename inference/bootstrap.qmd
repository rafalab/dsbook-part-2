...
The CLT provides a useful approach to building confidence intervals and performing hypothesis testing. However, it does not always apply. Here, we provide a short introduction to an alternative approach to estimating the distribution of an estimate that does not rely on CLT. 
...
```{r median-is-normal, message = FALSE, warning = FALSE, echo=FALSE}
m <- replicate(10^4, {
  x <- sample(income, N)
  median(x)
})
...
Note that a confidence interval constructed with the bootstrap is much closer to one constructed with the theoretical distribution:
