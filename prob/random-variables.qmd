...
This notation is convenient because it lets us express probability statements compactly. It may seem odd at first, since $X$ represents an outcome not yet observed, that we can discuss the likelihood of its possible values but not of its realized value. Once data are collected, we observe one realization of $X$, and we can use statistical inference to reason about what *could* have occurred, given what *did* occur.
...
For a numeric discrete random variable with possible outcomes $x_1,\dots,x_n$, the expected value is defined as:
...
The *standard error* (SE) describes the typical deviation of $X$ from its expected value. For a numeric discrete random variable, it is defined as:
...
If $X$ represents independent draws from the urn, then they all have the same expected value. Let's denote each expected value by $\mu_X$ and rewrite the equation as:
...
The given equation reveals crucial insights for practical scenarios. Specifically, it suggests that the standard error of the average of independent draws can be minimized by increasing the sample size, $n$, and we can quantify this reduction. However, this principle holds true only when the variables $X_1, X_2, ... X_n$ are independent. If they are not, the estimated standard error can be significantly off.
...
Any list of numbers $x_1, \dots, x_n$, or any dataset, has a distribution that describes how the observed values are spread out. We can summarize it with simple statistics such as the sample mean and standard deviation:
...
12\. Compute the standard error of $X$.
...
15\. What is the standard error of $S_n$?
