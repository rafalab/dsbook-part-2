...
So far, we have discussed the probability of events without taking into account any additional information from other events. In many applications, however, we want to update probabilities after learning that some other event has occurred. This leads to the idea of *conditional probability*.
...
We first note that there are 12 face cards in a 52-card deck and 
if the first card is an Ace, then 51 cards remain, still including all 12 face cards.
...
Conditional probability is fundamental in data science because it formalizes how probabilities change when new information becomes available. We will use this notation repeatedly in later chapters, especially when introducing Bayesian thinking, linear regression, and machine learning models.
