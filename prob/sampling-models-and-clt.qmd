...
4\. Use the CLT to estimate the probability that you end up winning money.
...
11\. Create a Monte Carlo simulation that generates 25,000 outcomes of $\bar{X}_n$, instead of $10,000$. Compute the average and standard deviation of the resulting list to confirm the results of 8, 9, and 10. Start your code by setting the seed to 1 with `set.seed(1)`.

12\. Now compare your Monte Carlo estimates of $\mathrm{Pr}(S_n>0)$ (exercise 6) and $\mathrm{Pr}(\bar{X}_n>0)$ (exercise 11) and their corresponding CLT approximations (exercises 4 and 10). Which of the following is true:
...
14\. Note that the total profit $S_n$ defined in the previous exercise is a random variable. Code a sampling model for $S_n$. Every time you run the sampling model code, you will obtain a different number of customers defaulting, which results in a different profit or loss.

15\. The expected value demonstrates that if you simply loan money to everybody without interest, you will end up losing money due to the 2% that defaults. Although you know 2% of your clients will probably default, you don't know which ones, so you can't remove them. Yet by charging everybody just a bit extra in interest, you can make up the losses incurred due to that 2%, and also cover your operating costs. What quantity $x$ would you have to charge each borrower so that your bank's expected profit is 0? Assume that you don't get $x$ from the borrowers that default. Also, note $x$ is not the interest rate, but the total you add, meaning $x/180000$ is the _interest rate_.
...
23\. How much do we have to increase $n$ by to ensure the probability of losing money is still less than 0.01?

24\. Confirm the result in exercise 23 with a Monte Carlo simulation.

25\. According to this equation, giving out more loans increases your expected profit and lowers the chances of losing money! Giving out more loans seems like a no-brainer. As a result, your colleague decides to leave your bank and start his own high-risk mortgage company. A few months later, your colleague's bank has gone bankrupt. A book is written, and eventually, the movies "The Big Short" and "Margin Call" are made, recounting the mistakes your friend and many others made. What happened?
...
26\. Use the simulation results of exercise 25 to report the expected profit, the probability of losing money, and the probability of losing more than $10,000,000. Study the distribution of profit and discuss how making the wrong assumption led to a catastrophic result.
