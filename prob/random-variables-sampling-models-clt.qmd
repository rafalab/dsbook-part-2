...
This particular random variable, which only takes values of 0 and 1, is referred to as a *Bernoulli trial*, with the 1s and 0s called successes and failures, respectively. Bernoulli trials appear frequently in practice because many data analysis projects involve counting how often something happens—this can be thought of as counting successes or, equivalently, adding Bernoulli random variables.
...
We call this a *sampling model*, as it involves modeling the random behavior of roulette through the sampling of draws from an urn. The total winnings $S_n$ is simply the sum of these 1,000 independent draws:
...
Keep in mind that if we can define a cumulative distribution function $F(a) = \mbox{Pr}(S_n\leq a)$, then we will be able to answer any question related to the probability of events defined by our random variable $S_n$, including the event $S_n<0$. We call this $F$ the random variable's *distribution function*.
...
s <- replicate(1000, roulette_winnings(n))
```
...
We can use the function `dbinom` and `pbinom` to compute the probabilities exactly. For example, to compute $\mbox{Pr}(S_n < 0)$, we note that:

$$\mbox{Pr}(S_n < 0) = \mbox{Pr}((S_n + n)/2 < (0+n)/2)$$

and we can use the `pbinom` to compute $$\mbox{Pr}(S_n \leq 0)$$:
...
Since this is a discrete probability function, to obtain $\mbox{Pr}(S_n < 0)$ rather than $\mbox{Pr}(S_n \leq 0)$, we write:
