...
When using a continuous distribution such as the normal, we no longer assign probabilities to individual points, each exact value has probability zero, but rather to *intervals*. In @sec-numeric-summaries we showed how this theoretical distributions can approximate.
