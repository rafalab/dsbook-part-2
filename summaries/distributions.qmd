...
When the data is continuous, the task of constructing a summary based on a distribution is more challenging because reporting the frequency of each entry is not an effective summary: because most entries are unique the relative frequency distribution does not summarize much.
...
Similar to what the frequency table does for categorical data, the eCDF defines the distribution for continuous data. This summary highlights key characteristics of the data. For example, about half of the states have populations over 5,000,000, most have fewer than 20,000,000, and similar insights.
...
Although this approach preserves all the information from the list of values, it can give misleading impressions if extrapolating insights to a broader population. For example, it might suggest there are no people with heights between 74 and 74.5 inches or that significantly more people are exactly 72 inches tall than 71.5 inches—patterns we know aren't accurate. In contrast, the eCDF provides a more useful summary:
...
The `hist` function allows us quickly contstuct a histogram. We can define custom bin intervals which is particularly useful for grouping extreme values and avoiding empty bins:
...
Discrete distributions also have means and standard deviations. We will discuss these in more detail @sec-XXX. For a list of values $x_1, \dots, x_n$ the mean, call it $\mu_x$ is defined by 
...
For the normal distribution, all the calculations related to quantiles are done without data, hence the name *theoretical quantiles*. But quantiles can be defined for any distribution, including an empirical one. If we therefore have data in a vector $x$, we can define the quantile associated with any proportion $p$ as the $q_p$ for which the proportion of values below $q_p$ is $p$.  In R you can compute the `q_p` associated with any `p` using the quantile function:
...
Notice that not all $p$ have a $q_p$ for which the proportion is exactly $p$. For these cases, there are several ways of defining the best $q_p$, as discussed in the help for the `quantile` function.
...
The idea of a qqplot is that if your data is well approximated by a normal distribution, then the quantiles of your data should be similar to the quantiles of a normal distribution. To construct a qqplot, we do the following:
...
plot(theoretical_quantiles, sample_quantiles); abline(0,1)
```
...
The above code is included to help describe qqplots. However, in practice it is easier to use the `qqnorm` function:
...
In the illustration above we used 20 quantiles; however, the default for the `qqnorm` and `geom_qq` function is to use as many quantiles as data points.

Although here we used qqplots to compare an observed distribution to the mathematically defined normal distribution, qqplots can be used to compare any two distributions. For example, here is qqplot comparing the distributions on males and females:
...
For that that follows a normal distribution, the *median* and  mean are the same, but this is generally not the case.
