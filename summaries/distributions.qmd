...
If we round the reported heights to the nearest inch and create a relative frequency distribution, we obtain a histogram with bins defined in one-inch intervals with the following **ggplot2** code:
...
The R base `hist` function allows us quickly construct a histogram. We can define custom bin intervals which is particularly useful for grouping extreme values and avoiding empty bins:
...
While the histogram is an assumption-free summary, the smoothed density relies on modeling assumptions. We must therefore choose the degree of smoothness with care, as it can meaningfully affect our interpretation of the data. The goal is to select a level of smoothing that we can justify as representative of the underlying distribution. In the case of height, for instance, we have good reason to expect that people with similar heights occur with similar frequencies: the proportion of individuals who are 72 inches tall should be closer to that of those at 71 inches than to those at 65 or 78 inches. This justifies a relatively smooth curve—one that resembles the example on the right more than the one on the left.
...
Here is a plot of the smooth density and the normal distribution approximation with mean = `r round(m,1)` and SD = `r round(s,1)` plotted as a black line with our student height smooth density in blue:
...
The normal distribution does appear to be quite a good approximation here. Later, in @sec-qqplots will see how well this approximation works at predicting the proportion of values within intervals.
...
## Quantile-quantile plots {#sec-qqplots}
...
For example, in the heights dataset, about 50% of men are shorter than 69.5 inches, so this is the median ($p=0.5$).

To compare, we make a qqplot to assess if our observations follow a normal distribution:
...
But instead of writing this by hand, R provides convenient functions:
...
:::{.callout-warning}
Because of this default behavior, large datasets can produce many plotted points that lie close to a line, resulting in heavy overplotting. In such cases, we recommend computing and plotting your own quantiles rather than relying on the default `qqplot` or `qqnorm` functions.
:::
...
```{r summaries-hist-qqplot-non-normal-data, echo = FALSE}
