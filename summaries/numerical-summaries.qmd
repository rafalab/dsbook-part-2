...
Although we introduced these concepts in the context of the normal distribution, the mean and standard deviation can be defined for any list of numbers, and they are commonly used to summarize data. In fact, all discrete distributions have means and standard deviations, and we will discuss these in more detail in @sec-random-variables. For now, we will focus on how to compute these summaries for a list of numbers and how to use them to summarize data. 
...
In @sec-population-sd we will see that, if we view our list of numbers as a sample from a normal distribution, then the version that divides by $n-1$ provides a better estimate of the population standard deviation. For now, we will use `sd(x)` to compute the standard deviation of the values in `x`.
:::
