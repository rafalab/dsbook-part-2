...
A relatively simple solution to QDA's problem of having too many parameters is to assume that the correlation structure is the same for all classes, which reduces the number of parameters we need to estimate.  In this case, the distributions looks like this:
...
So understanding how the normal density behaves helps explain why LDA and QDA behave like distance-based classifiers. For simplicity we consider the case in which classes are balanced: $\Pr(Y=k) = 1/K$ for all $k$:
...
which is the negative of the *Mahalanobis distance* squared.
...
The accuracy for the random forest fit for our 2 or 7 examples is 
