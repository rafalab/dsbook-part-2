...
The methods we have covered—linear regression, logistic regression, kNN, trees, and random forests, represent the statistical foundations of machine learning. They are interpretable, efficient, and effective for many real-world problems.

Modern deep learning methods take a different approach. Rather than assuming a specific functional form (like the linear models) or using explicit partitioning strategies (like trees), deep neural networks stack multiple layers of simple nonlinear transformations. Conceptually, this is a clever way to do smoothing: by composing many simple functions, deep networks can learn extremely flexible decision boundaries and automatically discover interactions between features. In the language of this book, deep networks allow features to interact in ways that are explicitly built into the model itself, rather than relying on methods like random forests that implicitly find interactions through partitioning.

We do not cover deep learning in this book. The statistical perspective we emphasize—focusing on bias-variance tradeoffs, regularization, and learning from data—remains relevant to deep learning, but the practice is quite different, involving specialized software, hardware acceleration, and techniques specific to training very large models. For those interested in deep learning, generative models, and large language models, there are dedicated textbooks and courses that build on the foundational concepts introduced here.
