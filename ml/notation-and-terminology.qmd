...
1\. You are given a dataset containing pairs $(x_i, y_i)$, where $x_i$ is a 2-dimensional feature vector describing the width and height of a handwritten digit image, and $y_i \in {0,1}$ is 1 if the digit is a "0".
...
* A prediction function that takes a new feature vector $x$ and predicts a score for class "0".
* A decision rule defined by a threshold $c$, written as a mathematical function.

2\. Suppose we want to build an algorithm that predicts whether a digit image is a "4". Let the image be summarized by a feature vector $x$, and let the prediction function be $\hat{f}(x)$, which returns a score between 0 and 1.
...
* Briefly explain in one or two sentences how changing the cutoff affects false positives (observations where $\hat{y}(x)=1$ when $y=0$) and false negatives (observations where $\hat{y}(x)=0$ when $y=1$).
...
* Define 0-1 loss as 0 for a correct prediction and 1 for an incorrect one. Write the 0–1 loss for a single observation ((x_i, y_i)).
* Write the average 0-1 loss over a test set of size (m).
