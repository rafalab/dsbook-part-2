...
Later we will learn that evaluating an algorithm on the training set can lead to _overfitting_, which often results in over-optimistic assessments, but for this simplistic example it is not much of a problem.
...
However, the estimate $\widehat{\text{MSE}}$ is a random variable. In fact, $\text{MSE}$ and $\widehat{\text{MSE}}$ are often referred to as the true error and apparent error, respectively.
...
9\. Consider different cutoffs for `cutoffs <- seq(0.05, 0.95, 0.05)` for this probability and generate an ROC curve based on the test set. Based on this ROC curve and the one computed in exercise 7, which method would you say performs better?
