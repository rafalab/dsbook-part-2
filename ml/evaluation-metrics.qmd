...
However, our cutoff-based approach is still better than guessing. And by testing on a dataset that we did not train on, we know our improvement is not due to overtraining.
...
6\. Using the results from exercise 5, find the best cutoff and compare the accuracy with the rule chosen exercise 4 on both `train_set` and `test_set`. 
...
9\. Consider different cutoffs for `cutofs <- seq(0.05, 0.95, 0.05)` for this probability and generate an ROC curve based on the test set. Based on this ROC curve and the one computed in exercise 7, which method would you say performs better?
