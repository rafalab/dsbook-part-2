...
Building machine learning models requires more than understanding the underlying mathematics.
The theory helps us avoid conceptual mistakes and choose appropriate methods, but turning these ideas into effective, real-world models involves additional practical considerations. We must decide how to preprocess and transform features, incorporate contextual knowledge to select relevant predictors, manage computational constraints, tune and compare algorithms, and often combine multiple models through techniques such as ensembling to boost predictive performance.

In this chapter, we bring these pieces together by walking through a complete case study using the MNIST handwritten-digit dataset, illustrating how the conceptual tools developed in earlier chapters translate into a practical machine learning workflow.

## Case study: handwritten digit recognition

We will use the MNIST dataset was introduced in @sec-mnist. We can load this data using the following **dslabs** package function:
...
Here we present an example where we try out 38 values between 1 and 75. To do this with __caret__, we need to define a column named `k`, so we use this:  `data.frame(k = seq(11, 105, 2))`. Note that when running this code, we are fitting 38 versions of kNN to 25 bootstrapped samples. Since we are fitting $38 \times 25 = 950$ kNN models, running this code will take several seconds.
...
:::{.callout-note}
Resampling methods involve random sampling steps, so repeated runs of the same code can produce different outcomes. To ensure that your results can be reproduced exactly, always set the random seed at the start of the analysis, as we did at the beginning of this chapter.
:::
...
So we end up removing `length(nzv)` = `r length(nzv)` predictors.
...
Additionally, the `train` function in caret includes a `preProcess` argument that allows users to specify which preprocessing steps to apply automatically during model training. We will explore this functionality further in the context of a k-Nearest Neighbors model in @sec-knn-in-practice.
...
In Section @sec-knn-cv-intro, we introduced the k-nearest neighbors (kNN) algorithm and described its intuition and mathematical formulation. Here, we focus on the practical aspects of applying kNN to real data. We will explore how to train and test kNN models in R, select an appropriate value of k, and evaluate performance using cross-validation and tuning techniques.
...
An alternative to removing low‐variance columns directly is to reduce the dimension of the feature matrix using Principal Component Analysis (PCA). For this dataset, a large fraction of the total variability in the predictors can be captured using only a small number of principal components (PCs).
...
We can now re-run our algorithm using only these `r p` transformed features:
...
In this example, we used the value of $k$ optimized for the raw data, not the PCA-transformed data. To obtain an unbiased MSE estimate when using PCA, we must recompute the PCA inside each cross-validation split and apply the resulting transformation to the corresponding validation fold.
...
```{r error-versus-number-of-trees, echo=-1}
rafalib::mypar()
plot(fit_rf)
```
