...
We start with a simple simulated example. Specifically, we simulate ratings $Y_{ij}$ for six movies and 120 users. For simplicity, we assume that these ratings have been adjusted for the movie and user effects, as described in [@sec-regularization]. We stored the results for 120 users and 6 movie effects in the object `y`:
...
To account for effects such as these, we use a latent factor model. Specifically, we extend the model from @sec-regularization to include factors that capture similarities between movies:
