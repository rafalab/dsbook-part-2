...
If the product of columns and rows does not match the length of the vector provided in the first argument, the `matrix` recycles values. If the length of the vector is a sub-multiple or multiple of the number of rows, this happens **without warning**:
...
The `nrow` function tells us how many rows the matrix has. Here is the number of rows `x` previously defined to store the MNIST training data:
...
We can use what we learned in @sec-row-col-summaries to compute this average for each pixel and display the values for each digit using a boxplot:
