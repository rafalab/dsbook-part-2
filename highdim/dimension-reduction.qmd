...
$$
with $\mathbf{z}_h$ and $\mathbf{z}_i$ the $h$-th and $i$-th $1 \times p$ rows, with $p=2$ in our specific example. Using linear algebra, we can rewrite this quantity as
...
Note that if $\mathbf{A} \mathbf{A}^\top = \mathbf{I}$ then the distance between the $h$th and $i$th rows is the same for the original and transformed data. We to transformation with the property $\mathbf{A} \mathbf{A}^\top = \mathbf{I}$ as _orthogonal transformations_ and they are guaranteed to preserves the distance between any two points.
...
With `z`, the proportion of the variability included in the first dimension is much higher. We can search for the rotation that maximizes the proportion of the variability included in the first dimension:
...
We can visualize these to see how the first component summarizes the data. In the plot below red represents high values and blue negative values (later, in Section @sec-factor-analysis), we learn why we call these weights and patterns:
