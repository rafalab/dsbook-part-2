...
r <- sweep(y_train - mu, 2, b_i_reg) - b_u
...
library(missMDA)
ind_x <- rowSums(!is.na(r)) > 25
ind_y <- colSums(!is.na(r[ind_x,])) > 10 #| colnames(r) %in% keep
dim(r[ind_x,ind_y])
pca <- imputePCA(r[ind_x,ind_y], ncp = 5)
pca$values

mean(!is.na(r[ind_x, ind_y]))
y <- y[ind_x, ind_y]
r <- r[ind_x, ind_y]
```
